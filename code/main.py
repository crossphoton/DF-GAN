...
def sampling(text_encoder, netG, dataloader, device):
    ...
    netG.load_state_dict(torch.load('models/%s/netG.pth' % (cfg.CONFIG_NAME),
                         map_location=torch.device("cuda" if cfg.CUDA else "cpu")))
    netG.eval()
...
if __name__ == "__main__":
    ...
    optimizerD = torch.optim.Adam(
        netD.parameters(), lr=0.0004, betas=(0.0, 0.9))

    print("Starting........\n\n\n\n\n\n")

    if cfg.B_VALIDATION:
        # generate images for the whole valid dataset
        count = sampling(text_encoder, netG, dataloader, device)
        print('state_epoch:  %d' % (state_epoch))
    else:

        count = train(dataloader, netG, netD, text_encoder,
